...
*   **Keyboard Target Selection**: Assigns selectable numbers from 1 to 9 to detected targets, then lets the operator focus tracking on one chosen target.
...
4.  **Target Numbering**: Each detected face/person tracker is assigned a visible selection number from 1 to 9. Additional targets beyond the configured limit are ignored.
5.  **Face Recognition**: The extracted facial embedding is compared against the embeddings of known profiles.
...
6.  **Liveness Detection for Unknown Faces**: If the face is not recognized, the system initiates a liveness check.
...
7.  **New Profile Promotion**: After a successful liveness check, the system starts collecting high-quality face samples of the new individual.
    *   Once enough samples are collected, a new profile is created and saved.
8.  **Focused Tracking Mode**: Pressing a number key from 1 to 9 selects that numbered target. Once a target is selected, other targets are no longer tracked, recognized, or saved to `known_faces`.
9.  **State Updates**: The application continuously updates a JSON file with the current state of all detected individuals (both known and unknown).
...
# Maximum number of selectable targets shown before keyboard selection (capped at 9)
SELECTABLE_TARGET_LIMIT=9
...
4.  **Select a target to track:**

    Detected targets are labeled with numbers from `[1]` to `[9]`. Press the matching number key to focus tracking on that target. After a target is selected, the application skips tracking, recognition, and `known_faces` updates for all other targets.

    Press `0` to clear the selection and return to numbered target detection mode.
